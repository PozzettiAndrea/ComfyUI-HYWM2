...
class HYWM2PLYAdvancedGaussianViewer(io.ComfyNode):
    ...
    def execute(cls, ply_path: str = ""):
        ...
        # Build a /view URL ComfyUI can serve (output / input / temp).
    ...
    def _build_view_url(ply_path: str) -> str:
        ...
        for kind, root in (
            ...
            return (
                f"/view?filename={filename}&type={kind}&subfolder={subfolder}"
            )
        # Fallback: serve by basename from output (best-effort).
